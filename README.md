...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/meps \
  -H 'Content-Type: application/json' \
  -d '{"country":"DEU","group":"EPP","term":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/meps`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "eu-parliament": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-eu-parliament"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-eu-parliament
```
